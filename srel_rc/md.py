...
cg_ff_file = ff_fmtstring % (fftype, LDCutIndex)
...
Prefix = md_fmtstring % (fftype, LDCutIndex)
Prefix = Prefix.split('_MD.lammpstrj.gz')[0]
...
Sys.Measures.Reset()
#Sys.Measures.VerboseOutput(Freq = NStep["Freq"])
#add a Lammps format trajectory dump action
Trj = sim.traj.lammps.LammpsWrite(FileName = Prefix + '_MD.lammpstrj.gz', MinImage = True)
