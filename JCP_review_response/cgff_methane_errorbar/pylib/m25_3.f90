!======MODULE CODE FOR m25_3======

subroutine calcenergyforces(ANumList, Pos, BoxL, AID, SID, MInd, BondOrdData, BondOrdStart, BondOrdShift, BondOrdLimit, CutSq, &
  & Param, Const, CalcForce, CalcDUParam, CalcDWParam, CalcTerms, PEnergy, Virial, Force, DUParam, DDUParam, DWParam, DDWParam, &
  & Terms, NList, NBondOrdData, NBondOrdStart, NBondOrdShift, NParam, NConst, NAtom, Dim, NDParam, NDDParam, NTerm)
    implicit none
...
!f2py intent(in, out) :: Terms
    ...
    real(8) :: LocalRho_2
    real(8) :: LocalRhoW_2
    real(8), dimension(0:NATOM-1,0:DIM-1) :: LocalRhoF_2
    ...
    do while ((NList == 0 .and. i < NAtom-1) .or. (ListIndi < NList))
        ...
        do j = 0, NAtom-1
            ...
            if ((AIDi==0) .and. (AIDj==0)) then
                if (dijsq < CutSq(2)) then
                    ...
                    if (DIJSQ < Const(368)) then
                        THISLOCALRHO = RhoSc_2
                        THISLOCALRHOF = 0.d0
                    else
                        THISLOCALRHO = RhoSc_2 * (Const(369) + DIJSQ * (Const(370) + DIJSQ * (Const(371) + Const(372) * DIJSQ)))
                        THISLOCALRHOF = RhoSc_2 * (Const(373) + DIJSQ * (Const(374) + DIJSQ * Const(375)))
                    endif
                    LocalRho_2 = LocalRho_2 + ThisLocalRho
                    LocalRhoW_2 = LocalRhoW_2 + (ThisLocalRhoF * dijsq)
                    if (CalcForce) then
                        ThisForce = ThisLocalRhoF * rij
                        LocalRhoF_2(j,:) = -ThisForce
                        LocalRhoF_2(i,:) = LocalRhoF_2(i,:) + ThisForce
                    endif
                end if
            end if
        ...
        if (AIDi==0) then
            ...
            LocalRho = LocalRho_2
            LocalRhoW = LocalRhoW_2
            SPx = (LOCALRHO - Const(365)) * Const(168)
            SPind = max(min(int(SPx), 48), 0)
            SPt = max(min(SPx - real(SPind), 1.d0), 0.d0)
            THISU = Const(169 + SPInd) + SPt * (Const(218 + SPInd) + SPt * (Const(267 + SPInd) + SPt * Const(316 + SPInd)))
            THISDU = (Const(218 + SPInd) + SPt * (2.*Const(267 + SPInd) + SPt * 3.*Const(316 + SPInd))) * Const(168)
            ...
            if (CalcDUParam) then
                ...
                if (SPind == 0) then
                    ...
                    DUParam(43 + SPInd) = DUParam(43 + SPInd) + SPd0
                    DUParam(43 + SPInd + 1) = DUParam(43 + SPInd + 1) + SPdp1 + SPdm1
                    DUParam(43 + SPInd + 2) = DUParam(43 + SPInd + 2) + SPdp2
                elseif (SPind == 48) then
                    !note y(N) = y(N-2) from zero slope condition
                    DUParam(43 + SPInd - 1) = DUParam(43 + SPInd - 1) + SPdm1
                    DUParam(43 + SPInd) = DUParam(43 + SPInd) + SPd0 + SPdp2
                    DUParam(43 + SPInd + 1) = DUParam(43 + SPInd + 1) + SPdp1
                else
                    DUParam(43 + SPInd - 1) = DUParam(43 + SPInd - 1) + SPdm1
                    DUParam(43 + SPInd) = DUParam(43 + SPInd) + SPd0
                    DUParam(43 + SPInd + 1) = DUParam(43 + SPInd + 1) + SPdp1
            ...
            if (CalcDWParam) then
                ...
                if (SPind == 0) then
                    !note y(-1) = y(1) from zero slope condition
                    DWParam(43 + SPInd) = DWParam(43 + SPInd) + SPd0
                    DWParam(43 + SPInd + 1) = DWParam(43 + SPInd + 1) + SPdp1 + SPdm1
                    DWParam(43 + SPInd + 2) = DWParam(43 + SPInd + 2) + SPdp2
                elseif (SPind == 48) then
                    !note y(N) = y(N-2) from zero slope condition
                    DWParam(43 + SPInd - 1) = DWParam(43 + SPInd - 1) + SPdm1
                    DWParam(43 + SPInd) = DWParam(43 + SPInd) + SPd0 + SPdp2
                    DWParam(43 + SPInd + 1) = DWParam(43 + SPInd + 1) + SPdp1
                else
                    DWParam(43 + SPInd - 1) = DWParam(43 + SPInd - 1) + SPdm1
                    DWParam(43 + SPInd) = DWParam(43 + SPInd) + SPd0
                    DWParam(43 + SPInd + 1) = DWParam(43 + SPInd + 1) + SPdp1
            ...
            if (CalcForce) then
                Force = Force + ThisdU * LocalRhoF_2
            endif
        end if
...
subroutine calcargstats(ANumList, Pos, BoxL, AID, SID, MInd, BondOrdData, BondOrdStart, BondOrdShift, BondOrdLimit, CutSq, &
  & Const, Weight, ArgMin, ArgMax, ArgAvg, ArgStd, ArgSum, ArgSumSq, ArgCount, NList, Dim, NAtom, NBondOrdData, NBondOrdStart, &
  & NBondOrdShift, NTerm, NConst, NArg)
    implicit none
    integer, intent(in) :: NList
    integer, intent(in) :: Dim
    integer, intent(in) :: NAtom
...
subroutine calcarghist(ANumList, Pos, BoxL, AID, SID, MInd, BondOrdData, BondOrdStart, BondOrdShift, BondOrdLimit, CutSq, &
  & Const, ArgHistMin, ArgHistiBinw, Weight, ArgHist, NList, Dim, NAtom, NBondOrdData, NBondOrdStart, NBondOrdShift, NTerm, &
  & NConst, NArg, NHist)
    implicit none
    integer, intent(in) :: NList
    integer, intent(in) :: Dim
    ...
    do ArgType = 0, 1 - 1
        do i = 0, 10000 - 1
            ...
            if (CalcDUParam) then
                ...
                if (SPind == 0) then
                    !note y(-1) = y(1) from zero slope condition
                    DUParam(43 + SPInd) = DUParam(43 + SPInd) + (ThisHist) * (SPd0)
                    DUParam(43 + SPInd + 1) = DUParam(43 + SPInd + 1) + (ThisHist) * (SPdp1 + SPdm1)
                    DUParam(43 + SPInd + 2) = DUParam(43 + SPInd + 2) + (ThisHist) * (SPdp2)
                elseif (SPind == 48) then
                    !note y(N) = y(N-2) from zero slope condition
                    DUParam(43 + SPInd - 1) = DUParam(43 + SPInd - 1) + (ThisHist) * (SPdm1)
                    DUParam(43 + SPInd) = DUParam(43 + SPInd) + (ThisHist) * (SPd0 + SPdp2)
                    DUParam(43 + SPInd + 1) = DUParam(43 + SPInd + 1) + (ThisHist) * (SPdp1)
                else
                    DUParam(43 + SPInd - 1) = DUParam(43 + SPInd - 1) + (ThisHist) * (SPdm1)
                    DUParam(43 + SPInd) = DUParam(43 + SPInd) + (ThisHist) * (SPd0)
                    DUParam(43 + SPInd + 1) = DUParam(43 + SPInd + 1) + (ThisHist) * (SPdp1)
            ...
            if (CalcDWParam) then
                ...
                if (SPind == 0) then
                    !note y(-1) = y(1) from zero slope condition
                    DWParam(43 + SPInd) = DWParam(43 + SPInd) + (ThisHist) * (SPd0)
                    DWParam(43 + SPInd + 1) = DWParam(43 + SPInd + 1) + (ThisHist) * (SPdp1 + SPdm1)
                    DWParam(43 + SPInd + 2) = DWParam(43 + SPInd + 2) + (ThisHist) * (SPdp2)
                elseif (SPind == 48) then
                    !note y(N) = y(N-2) from zero slope condition
                    DWParam(43 + SPInd - 1) = DWParam(43 + SPInd - 1) + (ThisHist) * (SPdm1)
                    DWParam(43 + SPInd) = DWParam(43 + SPInd) + (ThisHist) * (SPd0 + SPdp2)
                    DWParam(43 + SPInd + 1) = DWParam(43 + SPInd + 1) + (ThisHist) * (SPdp1)
                else
                    DWParam(43 + SPInd - 1) = DWParam(43 + SPInd - 1) + (ThisHist) * (SPdm1)
                    DWParam(43 + SPInd) = DWParam(43 + SPInd) + (ThisHist) * (SPd0)
                    DWParam(43 + SPInd + 1) = DWParam(43 + SPInd + 1) + (ThisHist) * (SPdp1)
...
!f2py intent(in, out) :: SDCCounts
    ...
    !measure DDUParam
    if (MasterI(119)==0) then
        UseMeasure(8) = .false.
    else
        if (MeasureAll) then
            UseMeasure(8) = .true.
            MasterF(8864:13060) = 0.d0
        elseif (MasterI(117) > 0 .and. mod(StepNum, MasterI(117))==0) then
            UseMeasure(8) = .true.
            MasterF(8864:13060) = 0.d0
        elseif (MasterI(118) > 0 .and. mod(CycleNum, MasterI(118))==0) then
            UseMeasure(8) = .true.
            MasterF(8864:13060) = 0.d0
        else
            UseMeasure(8) = .false.
    ...
    !measure DWParam
    if (MasterI(122)==0) then
        UseMeasure(9) = .false.
    else
        if (MeasureAll) then
            UseMeasure(9) = .true.
            MasterF(21456:21548) = 0.d0
        elseif (MasterI(120) > 0 .and. mod(StepNum, MasterI(120))==0) then
            UseMeasure(9) = .true.
            MasterF(21456:21548) = 0.d0
        elseif (MasterI(121) > 0 .and. mod(CycleNum, MasterI(121))==0) then
            UseMeasure(9) = .true.
            MasterF(21456:21548) = 0.d0
        else
            UseMeasure(9) = .false.
    ...
    !measure DDWParam
    if (MasterI(218)==0) then
        UseMeasure(10) = .false.
    else
        if (MeasureAll) then
            UseMeasure(10) = .true.
            MasterF(21736:25932) = 0.d0
        elseif (MasterI(216) > 0 .and. mod(StepNum, MasterI(216))==0) then
            UseMeasure(10) = .true.
            MasterF(21736:25932) = 0.d0
        elseif (MasterI(217) > 0 .and. mod(CycleNum, MasterI(217))==0) then
            UseMeasure(10) = .true.
            MasterF(21736:25932) = 0.d0
        else
            UseMeasure(10) = .false.
    ...
    if (MasterI(221)==0) then
        UseMeasure(11) = .false.
    else
        if (MeasureAll) then
            UseMeasure(11) = .true.
            MasterF(34328:42976) = 0.d0
        elseif (MasterI(219) > 0 .and. mod(StepNum, MasterI(219))==0) then
            UseMeasure(11) = .true.
            MasterF(34328:42976) = 0.d0
        elseif (MasterI(220) > 0 .and. mod(CycleNum, MasterI(220))==0) then
            UseMeasure(11) = .true.
            MasterF(34328:42976) = 0.d0
        else
            UseMeasure(11) = .false.
    ...
    !measure SDC
    if (MasterI(224)==0) then
        UseMeasure(12) = .false.
    else
        if (MeasureAll) then
            UseMeasure(12) = .true.
        elseif (MasterI(222) > 0 .and. mod(StepNum, MasterI(222))==0) then
            UseMeasure(12) = .true.
        elseif (MasterI(223) > 0 .and. mod(CycleNum, MasterI(223))==0) then
            UseMeasure(12) = .true.
        else
            UseMeasure(12) = .false.
        endif
    ...
    if (UseMeasure(7)) then
        !measure DUParam
        MasterF(28:120) = DUParam
        MasterI(24:116) = MasterI(24:116) + merge(0, 1, DUParam==0.d0)
        MasterF(121:213) = MasterF(121:213) + (MasterF(28:120))*Weight
        do v1 = 0, 93 - 1
            do v2 = 0, 93 - 1
                MasterF(214 + 93*v1 + v2) = MasterF(214 + 93*v1 + v2) + MasterF(28 + v1)*MasterF(28 + v2)*Weight
            enddo
        enddo
        MasterF(8863) = MasterF(8863) + Weight
    end if

    if (UseMeasure(8)) then
        !measure DDUParam
        MasterF(8864:13060) = DDUParam
        MasterF(13061:17257) = MasterF(13061:17257) + (MasterF(8864:13060))*Weight
        MasterF(17258:21454) = MasterF(17258:21454) + (MasterF(8864:13060))*(MasterF(8864:13060))*Weight
        MasterF(21455) = MasterF(21455) + Weight
    end if

    if (UseMeasure(9)) then
        !measure DWParam
        MasterF(21456:21548) = DWParam
        MasterI(123:215) = MasterI(123:215) + merge(0, 1, DWParam==0.d0)
        MasterF(21549:21641) = MasterF(21549:21641) + (MasterF(21456:21548))*Weight
        MasterF(21642:21734) = MasterF(21642:21734) + (MasterF(21456:21548))*(MasterF(21456:21548))*Weight
        MasterF(21735) = MasterF(21735) + Weight
    end if

    if (UseMeasure(10)) then
        !measure DDWParam
        MasterF(21736:25932) = DDWParam
        MasterF(25933:30129) = MasterF(25933:30129) + (MasterF(21736:25932))*Weight
        MasterF(30130:34326) = MasterF(30130:34326) + (MasterF(21736:25932))*(MasterF(21736:25932))*Weight
        MasterF(34327) = MasterF(34327) + Weight
    end if
    ...
    if (UseMeasure(11)) then
        ...
        do i = 0, NDParam - 1
            do j = 0, NDParam - 1
                MasterF(34328 + i*NDParam + j) = DUParam(i) * DWParam(j)
            enddo
        enddo
        MasterF(42977:51625) = MasterF(42977:51625) + (MasterF(34328:42976))*Weight
        MasterF(51626:60274) = MasterF(51626:60274) + (MasterF(34328:42976))*(MasterF(34328:42976))*Weight
        MasterF(60275) = MasterF(60275) + Weight
    end if
    ...
    if (UseMeasure(12)) then
        ...
        do SDCOr = 0, MasterI(227) - 1
            if (StepNum == MasterI(228) * SDCOr + MasterI(222)) then
                SDCPosOrigin(SDCOr, :, :) = Pos
            endif
            SDCInd = int((StepNum - MasterI(225) - SDCOr * MasterI(228)) / MasterI(222) - 1)
...
function modulehash()
    ...
    modulehash = '6ce571b265a99c7f18ef6300a693dc72996aef00'
end function
