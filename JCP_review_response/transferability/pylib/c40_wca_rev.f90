...
!f2py intent(in, out) :: Terms
    ...
    real(8) :: LocalRho_4
    real(8) :: LocalRhoW_4
    real(8), dimension(0:NATOM-1,0:DIM-1) :: LocalRhoF_4
    integer, dimension(114), parameter :: Apply3_0 = (/ 0,1,2,1,2,3,2,3,4,3,4,5,4,5,6,5,6,7,6,7,8,7,8,9,8,9,10,9,10,11,10,11,12 &
      & ,11,12,13,12,13,14,13,14,15,14,15,16,15,16,17,16,17,18,17,18,19,18,19,20,19,20,21,20,21,22,21,22,23,22,23,24,23,24,25  &
      & ,24,25,26,25,26,27,26,27,28,27,28,29,28,29,30,29,30,31,30,31,32,31,32,33,32,33,34,33,34,35,34,35,36,35,36,37,36,37,38  &
      & ,37,38,39 /)
    ...
    do while ((NList == 0 .and. i < NAtom-1) .or. (ListIndi < NList))
        ...
        do j = 0, NAtom-1
            ...
            if (i < j) then
                if (BondOrdij>=5) then
                    if (dijsq < CutSq(0)) then
                        !pair interactions for potential <WCA_AA>
                        Sig = Param(2)
                        Eps = Param(1)
                        idist2 = Sig**2 / DIJSQ
                        ...
                        if (CalcDUParam) then
                            DUParam(1) = DUParam(1) + val1 + Const(1)
                            val1 = 1.d0 / Sig
                            DUParam(2) = DUParam(2) - THISW * val1 + Const(2)
                            DDUParam(4) = DDUParam(4) + Eps * (528.d0*idist12 - 120.d0*idist6) * val1*val1 + Const(3)
                            DDUParam(2) = DDUParam(2) - val2 * val1 + Const(4)
                        endif
                        if (CalcDWParam) then
                            DWParam(1) = DWParam(1) + val2
                            val1 = 1.d0 / Sig
                            val3 = (144.d0 * idist6 - 576.d0 * idist12 ) * val1
                            DWParam(2) = DWParam(2) + Eps *  val3
                            DDWParam(4) = DDWParam(4) + Eps * (720.d0*idist6 - 6336.d0*idist12) * val1*val1
                            DDWParam(2) = DDWParam(2) + val3
                        endif
                    end if
                end if
            end if
            ...
            if (i < j) then
                if (BondOrdij>=5) then
                    if (dijsq < CutSq(1)) then
                        ...
                        SPt = SPx - real(SPind)
                        THISU = Const(7 + SPInd) + SPt * (Const(47 + SPInd) + SPt * (Const(87 + SPInd) + SPt * &
                          & Const(127 + SPInd)))
                        THISW = (Const(47 + SPInd) + SPt * (2.*Const(87 + SPInd) + SPt * 3.*Const(127 + SPInd))) * SPx
                        PEnergy = PEnergy + ThisU
                        Virial = Virial + ThisW
                        ThisWsum = ThisWsum + ThisW
                        if (CalcTerms) then
                        ...
                        if (CalcDUParam) then
                            ...
                            if (SPind == 0) then
                                ...
                                DUParam(3 + SPInd) = DUParam(3 + SPInd) + SPd0 + 2.0d0 * SPdm1
                                DUParam(3 + SPInd + 1) = DUParam(3 + SPInd + 1) + SPdp1 - SPdm1
                                DUParam(3 + SPInd + 2) = DUParam(3 + SPInd + 2) + SPdp2
                            elseif (SPind == 39) then
                                !note: y(N) = -0.5 y(N-1) and y(N+1) = y(N-1) for zero slope and value condition
                                DUParam(3 + SPInd - 1) = DUParam(3 + SPInd - 1) + SPdm1
                                DUParam(3 + SPInd) = DUParam(3 + SPInd) + SPd0 - 0.5d0 * SPdp1 + SPdp2
                            elseif (SPind == 38) then
                                !note: y(N) = -0.5 y(N-1) and y(N+1) = y(N-1) for zero slope and value condition
                                DUParam(3 + SPInd - 1) = DUParam(3 + SPInd - 1) + SPdm1
                                DUParam(3 + SPInd) = DUParam(3 + SPInd) + SPd0
                                DUParam(3 + SPInd + 1) = DUParam(3 + SPInd + 1) + SPdp1 - 0.5d0 * SPdp2
                            else
                                DUParam(3 + SPInd - 1) = DUParam(3 + SPInd - 1) + SPdm1
                                DUParam(3 + SPInd) = DUParam(3 + SPInd) + SPd0
                                DUParam(3 + SPInd + 1) = DUParam(3 + SPInd + 1) + SPdp1
                                DUParam(3 + SPInd + 2) = DUParam(3 + SPInd + 2) + SPdp2
                            endif
                        endif
                        if (CalcDWParam) then
                            ...
                            if (SPind == 0) then
                                !note: y(-1) = 2 y(0) - y(1) for linearity condition
                                DWParam(3 + SPInd) = DWParam(3 + SPInd) + SPd0 + 2.0d0 * SPdm1
                                DWParam(3 + SPInd + 1) = DWParam(3 + SPInd + 1) + SPdp1 - SPdm1
                                DWParam(3 + SPInd + 2) = DWParam(3 + SPInd + 2) + SPdp2
                            elseif (SPind == 39) then
                                !note: y(N) = -0.5 y(N-1) and y(N+1) = y(N-1) for zero slope and value condition
                                DWParam(3 + SPInd - 1) = DWParam(3 + SPInd - 1) + SPdm1
                                DWParam(3 + SPInd) = DWParam(3 + SPInd) + SPd0 - 0.5d0 * SPdp1 + SPdp2
                            elseif (SPind == 38) then
                                !note: y(N) = -0.5 y(N-1) and y(N+1) = y(N-1) for zero slope and value condition
                                DWParam(3 + SPInd - 1) = DWParam(3 + SPInd - 1) + SPdm1
                                DWParam(3 + SPInd) = DWParam(3 + SPInd) + SPd0
                                DWParam(3 + SPInd + 1) = DWParam(3 + SPInd + 1) + SPdp1 - 0.5d0 * SPdp2
                            else
                                DWParam(3 + SPInd - 1) = DWParam(3 + SPInd - 1) + SPdm1
                                DWParam(3 + SPInd) = DWParam(3 + SPInd) + SPd0
                                DWParam(3 + SPInd + 1) = DWParam(3 + SPInd + 1) + SPdp1
                                DWParam(3 + SPInd + 2) = DWParam(3 + SPInd + 2) + SPdp2
                            endif
                        endif
                    end if
            ...
            if (i < j) then
                if (Bonded) then
                    if (dijsq < CutSq(2)) then
                        ...
                        val1 = DIJ - Param(43)
                        THISU = Param(44) * val1*val1
                        THISW = 2.d0 * Param(44) * val1*DIJ
                        PEnergy = PEnergy + ThisU
                        Virial = Virial + ThisW
                        ThisWsum = ThisWsum + ThisW
                        if (CalcTerms) then
                        ...
                        if (CalcDUParam) then
                            DUParam(44) = DUParam(44) + val1 * val1
                            DUParam(43) = DUParam(43) - 2.d0 * Param(44) * val1
                            DDUParam(1605) = DDUParam(1605) + 2.d0 * Param(44)
                            DDUParam(1607) = DDUParam(1607) - 2.d0 * val1
                        endif
                        if (CalcDWParam) then
                            DWParam(44) = DWParam(44) + 2.d0 * val1 * DIJ
                            DWParam(43) = DWParam(43) - 2.d0 * Param(44) * DIJ
                            DDWParam(1607) = DDWParam(1607) - 2.d0 * DIJ
                        endif
                    end if
                end if
            end if
            ...
            if ((AIDi==0) .and. (AIDj==0)) then
                if (dijsq < CutSq(4)) then
                    ...
                    if (DIJSQ < Const(272)) then
                        THISLOCALRHO = RhoSc_4
                        THISLOCALRHOF = 0.d0
                    else
                        THISLOCALRHO = RhoSc_4 * (Const(273) + DIJSQ * (Const(274) + DIJSQ * (Const(275) + Const(276) * DIJSQ)))
                        THISLOCALRHOF = RhoSc_4 * (Const(277) + DIJSQ * (Const(278) + DIJSQ * Const(279)))
                    endif
                    LocalRho_4 = LocalRho_4 + ThisLocalRho
                    LocalRhoW_4 = LocalRhoW_4 + (ThisLocalRhoF * dijsq)
                    if (CalcForce) then
                        ThisForce = ThisLocalRhoF * rij
                        LocalRhoF_4(j,:) = -ThisForce
                        LocalRhoF_4(i,:) = LocalRhoF_4(i,:) + ThisForce
                    endif
                end if
            end if
        ...
        if (AIDi==0) then
            ...
            LocalRho = LocalRho_4
            LocalRhoW = LocalRhoW_4
            SPx = (LOCALRHO - Const(269)) * Const(168)
            SPind = max(min(int(SPx), 24), 0)
            SPt = max(min(SPx - real(SPind), 1.d0), 0.d0)
            THISU = Const(169 + SPInd) + SPt * (Const(194 + SPInd) + SPt * (Const(219 + SPInd) + SPt * Const(244 + SPInd)))
            THISDU = (Const(194 + SPInd) + SPt * (2.*Const(219 + SPInd) + SPt * 3.*Const(244 + SPInd))) * Const(168)
            ThisW = ThisdU * LocalRhoW
            PEnergy = PEnergy + ThisU
            Virial = Virial + ThisW
            if (CalcTerms) then
                Terms(4) = Terms(4) + ThisU
            endif
            if (CalcDUParam) then
                SPdm1 = 0.166666666666667d0 + SPt * (-0.5d0 + SPt * (0.5d0 - SPt * 0.166666666666667d0))
                SPd0 = 0.666666666666667d0 + SPt * SPt * (-1.0d0 + SPt * 0.5d0)
                ...
                if (SPind == 0) then
                    ...
                    DUParam(47 + SPInd) = DUParam(47 + SPInd) + SPd0
                    DUParam(47 + SPInd + 1) = DUParam(47 + SPInd + 1) + SPdp1 + SPdm1
                    DUParam(47 + SPInd + 2) = DUParam(47 + SPInd + 2) + SPdp2
                elseif (SPind == 24) then
                    !note y(N) = y(N-2) from zero slope condition
                    DUParam(47 + SPInd - 1) = DUParam(47 + SPInd - 1) + SPdm1
                    DUParam(47 + SPInd) = DUParam(47 + SPInd) + SPd0 + SPdp2
                    DUParam(47 + SPInd + 1) = DUParam(47 + SPInd + 1) + SPdp1
                else
                    DUParam(47 + SPInd - 1) = DUParam(47 + SPInd - 1) + SPdm1
                    DUParam(47 + SPInd) = DUParam(47 + SPInd) + SPd0
                    DUParam(47 + SPInd + 1) = DUParam(47 + SPInd + 1) + SPdp1
            ...
            if (CalcDWParam) then
                ...
                if (SPind == 0) then
                    !note y(-1) = y(1) from zero slope condition
                    DWParam(47 + SPInd) = DWParam(47 + SPInd) + SPd0
                    DWParam(47 + SPInd + 1) = DWParam(47 + SPInd + 1) + SPdp1 + SPdm1
                    DWParam(47 + SPInd + 2) = DWParam(47 + SPInd + 2) + SPdp2
                elseif (SPind == 24) then
                    !note y(N) = y(N-2) from zero slope condition
                    DWParam(47 + SPInd - 1) = DWParam(47 + SPInd - 1) + SPdm1
                    DWParam(47 + SPInd) = DWParam(47 + SPInd) + SPd0 + SPdp2
                    DWParam(47 + SPInd + 1) = DWParam(47 + SPInd + 1) + SPdp1
                else
                    DWParam(47 + SPInd - 1) = DWParam(47 + SPInd - 1) + SPdm1
                    DWParam(47 + SPInd) = DWParam(47 + SPInd) + SPd0
                    DWParam(47 + SPInd + 1) = DWParam(47 + SPInd + 1) + SPdp1
            ...
            if (CalcForce) then
                Force = Force + ThisdU * LocalRhoF_4
            endif
        end if
    ...
    do m = 0, NMol-1
        ...
        do ApInd = ApInd1, ApInd2, 3
            ...
            dTheta = THETA - Param(45)
            THISU = Param(46) * dTheta*dTheta
            PEnergy = PEnergy + ThisU
            if (CalcForce .and. sintheta /= 0.d0) then
                THISDU = 2. * Param(46) * dTheta
            ...
            if (CalcDUParam) then
                DUParam(46) = DUParam(46) + dTheta * dTheta
                DUParam(45) = DUParam(45) - 2. * Param(46) * dTheta
                DDUParam(1609) = DDUParam(1609) - 2. * Param(46)
                DDUParam(1610) = DDUParam(1610) - 2. * dTheta
            endif
            if (CalcDWParam) then
            endif
        enddo
...
!f2py intent(in, out) :: ArgCount
    ...
    !initialization for potential <WCA_AA>
    ...
    do while ((NList == 0 .and. i < NAtom-1) .or. (ListIndi < NList))
        ...
        ARGTYPE = 0
        if (Weight > 0.d0) then
            ArgMin(4 + ArgType) = min(ArgMin(4 + ArgType), ArgVal)
            ArgMax(4 + ArgType) = max(ArgMax(4 + ArgType), ArgVal)
            ArgCount(4 + ArgType) = ArgCount(4 + ArgType) + Weight
            ArgSum(4 + ArgType) = ArgSum(4 + ArgType) + ArgVal * Weight
            ArgSumSq(4 + ArgType) = ArgSumSq(4 + ArgType) + ArgVal * ArgVal * Weight
        endif
...
!f2py intent(in, out) :: ArgHist
    ...
    do while ((NList == 0 .and. i < NAtom-1) .or. (ListIndi < NList))
        ...
        ARGTYPE = 0
        if (10000 > 0) then
            BinInd = int((ArgVal - ArgHistMin(4 + ArgType)) * ArgHistiBinw(4 + ArgType))
            if (BinInd >= 0 .and. BinInd < 10000) ArgHist(40000 + 10000*ArgType + BinInd) = &
              & ArgHist(40000 + 10000*ArgType + BinInd) + Weight
        endif
...
!f2py intent(in, out) :: Terms
    ...
    do ArgType = 0, 1 - 1
        do i = 0, 10000 - 1
            ...
            if (CalcDUParam) then
                DUParam(1) = DUParam(1) + (ThisHist) * (val1 + Const(1))
                val1 = 1.d0 / Sig
                DUParam(2) = DUParam(2) + (ThisHist) * (-THISW * val1 + Const(2))
                DDUParam(4) = DDUParam(4) + (ThisHist) * (Eps * (528.d0*idist12 - 120.d0*idist6) * val1*val1 + Const(3))
                DDUParam(2) = DDUParam(2) + (ThisHist) * (-val2 * val1 + Const(4))
            endif
            if (CalcDWParam) then
                DWParam(1) = DWParam(1) + (ThisHist) * (val2)
                val1 = 1.d0 / Sig
                val3 = (144.d0 * idist6 - 576.d0 * idist12 ) * val1
                DWParam(2) = DWParam(2) + (ThisHist) * (Eps *  val3)
                DDWParam(4) = DDWParam(4) + (ThisHist) * (Eps * (720.d0*idist6 - 6336.d0*idist12) * val1*val1)
                DDWParam(2) = DDWParam(2) + (ThisHist) * (val3)
            endif
        enddo
    enddo
    ...
    do ArgType = 0, 1 - 1
        do i = 0, 10000 - 1
            ...
            SPind = max(min(int(SPx), 39), 0)
            SPt = SPx - real(SPind)
            THISU = Const(7 + SPInd) + SPt * (Const(47 + SPInd) + SPt * (Const(87 + SPInd) + SPt * Const(127 + SPInd)))
            THISW = (Const(47 + SPInd) + SPt * (2.*Const(87 + SPInd) + SPt * 3.*Const(127 + SPInd))) * SPx
            PEnergy = PEnergy + ThisU * ThisHist
            if (CalcTerms) then
            ...
            if (CalcDUParam) then
                ...
                if (SPind == 0) then
                    !note: y(-1) = 2 y(0) - y(1) for linearity condition
                    DUParam(3 + SPInd) = DUParam(3 + SPInd) + (ThisHist) * (SPd0 + 2.0d0 * SPdm1)
                    DUParam(3 + SPInd + 1) = DUParam(3 + SPInd + 1) + (ThisHist) * (SPdp1 - SPdm1)
                    DUParam(3 + SPInd + 2) = DUParam(3 + SPInd + 2) + (ThisHist) * (SPdp2)
                elseif (SPind == 39) then
                    !note: y(N) = -0.5 y(N-1) and y(N+1) = y(N-1) for zero slope and value condition
                    DUParam(3 + SPInd - 1) = DUParam(3 + SPInd - 1) + (ThisHist) * (SPdm1)
                    DUParam(3 + SPInd) = DUParam(3 + SPInd) + (ThisHist) * (SPd0 - 0.5d0 * SPdp1 + SPdp2)
                elseif (SPind == 38) then
                    !note: y(N) = -0.5 y(N-1) and y(N+1) = y(N-1) for zero slope and value condition
                    DUParam(3 + SPInd - 1) = DUParam(3 + SPInd - 1) + (ThisHist) * (SPdm1)
                    DUParam(3 + SPInd) = DUParam(3 + SPInd) + (ThisHist) * (SPd0)
                    DUParam(3 + SPInd + 1) = DUParam(3 + SPInd + 1) + (ThisHist) * (SPdp1 - 0.5d0 * SPdp2)
                else
                    DUParam(3 + SPInd - 1) = DUParam(3 + SPInd - 1) + (ThisHist) * (SPdm1)
                    DUParam(3 + SPInd) = DUParam(3 + SPInd) + (ThisHist) * (SPd0)
                    DUParam(3 + SPInd + 1) = DUParam(3 + SPInd + 1) + (ThisHist) * (SPdp1)
                    DUParam(3 + SPInd + 2) = DUParam(3 + SPInd + 2) + (ThisHist) * (SPdp2)
                endif
            endif
            if (CalcDWParam) then
                ...
                if (SPind == 0) then
                    !note: y(-1) = 2 y(0) - y(1) for linearity condition
                    DWParam(3 + SPInd) = DWParam(3 + SPInd) + (ThisHist) * (SPd0 + 2.0d0 * SPdm1)
                    DWParam(3 + SPInd + 1) = DWParam(3 + SPInd + 1) + (ThisHist) * (SPdp1 - SPdm1)
                    DWParam(3 + SPInd + 2) = DWParam(3 + SPInd + 2) + (ThisHist) * (SPdp2)
                elseif (SPind == 39) then
                    !note: y(N) = -0.5 y(N-1) and y(N+1) = y(N-1) for zero slope and value condition
                    DWParam(3 + SPInd - 1) = DWParam(3 + SPInd - 1) + (ThisHist) * (SPdm1)
                    DWParam(3 + SPInd) = DWParam(3 + SPInd) + (ThisHist) * (SPd0 - 0.5d0 * SPdp1 + SPdp2)
                elseif (SPind == 38) then
                    !note: y(N) = -0.5 y(N-1) and y(N+1) = y(N-1) for zero slope and value condition
                    DWParam(3 + SPInd - 1) = DWParam(3 + SPInd - 1) + (ThisHist) * (SPdm1)
                    DWParam(3 + SPInd) = DWParam(3 + SPInd) + (ThisHist) * (SPd0)
                    DWParam(3 + SPInd + 1) = DWParam(3 + SPInd + 1) + (ThisHist) * (SPdp1 - 0.5d0 * SPdp2)
                else
                    DWParam(3 + SPInd - 1) = DWParam(3 + SPInd - 1) + (ThisHist) * (SPdm1)
                    DWParam(3 + SPInd) = DWParam(3 + SPInd) + (ThisHist) * (SPd0)
                    DWParam(3 + SPInd + 1) = DWParam(3 + SPInd + 1) + (ThisHist) * (SPdp1)
                    DWParam(3 + SPInd + 2) = DWParam(3 + SPInd + 2) + (ThisHist) * (SPdp2)
                endif
            endif
        enddo
    ...
    do ArgType = 0, 1 - 1
        do i = 0, 10000 - 1
            ...
            if (CalcDUParam) then
                DUParam(44) = DUParam(44) + (ThisHist) * (val1 * val1)
                DUParam(43) = DUParam(43) + (ThisHist) * (-2.d0 * Param(44) * val1)
                DDUParam(1605) = DDUParam(1605) + (ThisHist) * (2.d0 * Param(44))
                DDUParam(1607) = DDUParam(1607) + (ThisHist) * (-2.d0 * val1)
            endif
            if (CalcDWParam) then
                DWParam(44) = DWParam(44) + (ThisHist) * (2.d0 * val1 * DIJ)
                DWParam(43) = DWParam(43) + (ThisHist) * (-2.d0 * Param(44) * DIJ)
                DDWParam(1607) = DDWParam(1607) + (ThisHist) * (-2.d0 * DIJ)
            endif
        enddo
    enddo
    ...
    do ArgType = 0, 1 - 1
        do i = 0, 10000 - 1
            ...
            if (CalcDUParam) then
                DUParam(46) = DUParam(46) + (ThisHist) * (dTheta * dTheta)
                DUParam(45) = DUParam(45) + (ThisHist) * (-2. * Param(46) * dTheta)
                DDUParam(1609) = DDUParam(1609) + (ThisHist) * (-2. * Param(46))
                DDUParam(1610) = DDUParam(1610) + (ThisHist) * (-2. * dTheta)
            endif
            if (CalcDWParam) then
            endif
        enddo
    ...
    do ArgType = 0, 1 - 1
        do i = 0, 10000 - 1
            ThisHist = ArgHist(40000 + 10000*ArgType + i)
            ArgVal = ArgHistMin(4 + ArgType) + ArgHistBinw(4 + ArgType) * (0.5d0 + i)
            LOCALRHO = ARGVAL
            SPx = (LOCALRHO - Const(269)) * Const(168)
            SPind = max(min(int(SPx), 24), 0)
            SPt = max(min(SPx - real(SPind), 1.d0), 0.d0)
            ...
            if (CalcDUParam) then
                ...
                if (SPind == 0) then
                    !note y(-1) = y(1) from zero slope condition
                    DUParam(47 + SPInd) = DUParam(47 + SPInd) + (ThisHist) * (SPd0)
                    DUParam(47 + SPInd + 1) = DUParam(47 + SPInd + 1) + (ThisHist) * (SPdp1 + SPdm1)
                    DUParam(47 + SPInd + 2) = DUParam(47 + SPInd + 2) + (ThisHist) * (SPdp2)
                elseif (SPind == 24) then
                    !note y(N) = y(N-2) from zero slope condition
                    DUParam(47 + SPInd - 1) = DUParam(47 + SPInd - 1) + (ThisHist) * (SPdm1)
                    DUParam(47 + SPInd) = DUParam(47 + SPInd) + (ThisHist) * (SPd0 + SPdp2)
                    DUParam(47 + SPInd + 1) = DUParam(47 + SPInd + 1) + (ThisHist) * (SPdp1)
                else
                    DUParam(47 + SPInd - 1) = DUParam(47 + SPInd - 1) + (ThisHist) * (SPdm1)
                    DUParam(47 + SPInd) = DUParam(47 + SPInd) + (ThisHist) * (SPd0)
                    DUParam(47 + SPInd + 1) = DUParam(47 + SPInd + 1) + (ThisHist) * (SPdp1)
            ...
            if (CalcDWParam) then
                ...
                if (SPind == 0) then
                    !note y(-1) = y(1) from zero slope condition
                    DWParam(47 + SPInd) = DWParam(47 + SPInd) + (ThisHist) * (SPd0)
                    DWParam(47 + SPInd + 1) = DWParam(47 + SPInd + 1) + (ThisHist) * (SPdp1 + SPdm1)
                    DWParam(47 + SPInd + 2) = DWParam(47 + SPInd + 2) + (ThisHist) * (SPdp2)
                elseif (SPind == 24) then
                    !note y(N) = y(N-2) from zero slope condition
                    DWParam(47 + SPInd - 1) = DWParam(47 + SPInd - 1) + (ThisHist) * (SPdm1)
                    DWParam(47 + SPInd) = DWParam(47 + SPInd) + (ThisHist) * (SPd0 + SPdp2)
                    DWParam(47 + SPInd + 1) = DWParam(47 + SPInd + 1) + (ThisHist) * (SPdp1)
                else
                    DWParam(47 + SPInd - 1) = DWParam(47 + SPInd - 1) + (ThisHist) * (SPdm1)
                    DWParam(47 + SPInd) = DWParam(47 + SPInd) + (ThisHist) * (SPd0)
                    DWParam(47 + SPInd + 1) = DWParam(47 + SPInd + 1) + (ThisHist) * (SPdp1)
...
!f2py intent(in, out) :: SDCCounts
    ...
    !measure DDUParam
    if (MasterI(99)==0) then
        UseMeasure(8) = .false.
    else
        if (MeasureAll) then
            UseMeasure(8) = .true.
            MasterF(5504:7864) = 0.d0
        elseif (MasterI(97) > 0 .and. mod(StepNum, MasterI(97))==0) then
            UseMeasure(8) = .true.
            MasterF(5504:7864) = 0.d0
        elseif (MasterI(98) > 0 .and. mod(CycleNum, MasterI(98))==0) then
            UseMeasure(8) = .true.
            MasterF(5504:7864) = 0.d0
        else
            UseMeasure(8) = .false.
    ...
    !measure DWParam
    if (MasterI(102)==0) then
        UseMeasure(9) = .false.
    else
        if (MeasureAll) then
            UseMeasure(9) = .true.
            MasterF(12588:12660) = 0.d0
        elseif (MasterI(100) > 0 .and. mod(StepNum, MasterI(100))==0) then
            UseMeasure(9) = .true.
            MasterF(12588:12660) = 0.d0
        elseif (MasterI(101) > 0 .and. mod(CycleNum, MasterI(101))==0) then
            UseMeasure(9) = .true.
            MasterF(12588:12660) = 0.d0
        else
            UseMeasure(9) = .false.
    ...
    !measure DDWParam
    if (MasterI(178)==0) then
        UseMeasure(10) = .false.
    else
        if (MeasureAll) then
            UseMeasure(10) = .true.
            MasterF(12808:15168) = 0.d0
        elseif (MasterI(176) > 0 .and. mod(StepNum, MasterI(176))==0) then
            UseMeasure(10) = .true.
            MasterF(12808:15168) = 0.d0
        elseif (MasterI(177) > 0 .and. mod(CycleNum, MasterI(177))==0) then
            UseMeasure(10) = .true.
            MasterF(12808:15168) = 0.d0
        else
            UseMeasure(10) = .false.
    ...
    if (MasterI(181)==0) then
        UseMeasure(11) = .false.
    else
        if (MeasureAll) then
            UseMeasure(11) = .true.
            MasterF(19892:25220) = 0.d0
        elseif (MasterI(179) > 0 .and. mod(StepNum, MasterI(179))==0) then
            UseMeasure(11) = .true.
            MasterF(19892:25220) = 0.d0
        elseif (MasterI(180) > 0 .and. mod(CycleNum, MasterI(180))==0) then
            UseMeasure(11) = .true.
            MasterF(19892:25220) = 0.d0
        else
            UseMeasure(11) = .false.
    ...
    !measure SDC
    if (MasterI(184)==0) then
        UseMeasure(12) = .false.
    else
        if (MeasureAll) then
            UseMeasure(12) = .true.
        elseif (MasterI(182) > 0 .and. mod(StepNum, MasterI(182))==0) then
            UseMeasure(12) = .true.
        elseif (MasterI(183) > 0 .and. mod(CycleNum, MasterI(183))==0) then
            UseMeasure(12) = .true.
        else
            UseMeasure(12) = .false.
        endif
    ...
    if (UseMeasure(7)) then
        !measure DUParam
        MasterF(28:100) = DUParam
        MasterI(24:96) = MasterI(24:96) + merge(0, 1, DUParam==0.d0)
        MasterF(101:173) = MasterF(101:173) + (MasterF(28:100))*Weight
        do v1 = 0, 73 - 1
            do v2 = 0, 73 - 1
                MasterF(174 + 73*v1 + v2) = MasterF(174 + 73*v1 + v2) + MasterF(28 + v1)*MasterF(28 + v2)*Weight
            enddo
        enddo
        MasterF(5503) = MasterF(5503) + Weight
    end if

    if (UseMeasure(8)) then
        !measure DDUParam
        MasterF(5504:7864) = DDUParam
        MasterF(7865:10225) = MasterF(7865:10225) + (MasterF(5504:7864))*Weight
        MasterF(10226:12586) = MasterF(10226:12586) + (MasterF(5504:7864))*(MasterF(5504:7864))*Weight
        MasterF(12587) = MasterF(12587) + Weight
    end if

    if (UseMeasure(9)) then
        !measure DWParam
        MasterF(12588:12660) = DWParam
        MasterI(103:175) = MasterI(103:175) + merge(0, 1, DWParam==0.d0)
        MasterF(12661:12733) = MasterF(12661:12733) + (MasterF(12588:12660))*Weight
        MasterF(12734:12806) = MasterF(12734:12806) + (MasterF(12588:12660))*(MasterF(12588:12660))*Weight
        MasterF(12807) = MasterF(12807) + Weight
    end if

    if (UseMeasure(10)) then
        !measure DDWParam
        MasterF(12808:15168) = DDWParam
        MasterF(15169:17529) = MasterF(15169:17529) + (MasterF(12808:15168))*Weight
        MasterF(17530:19890) = MasterF(17530:19890) + (MasterF(12808:15168))*(MasterF(12808:15168))*Weight
        MasterF(19891) = MasterF(19891) + Weight
    end if
    ...
    if (UseMeasure(11)) then
        ...
        do i = 0, NDParam - 1
            do j = 0, NDParam - 1
                MasterF(19892 + i*NDParam + j) = DUParam(i) * DWParam(j)
            enddo
        enddo
        MasterF(25221:30549) = MasterF(25221:30549) + (MasterF(19892:25220))*Weight
        MasterF(30550:35878) = MasterF(30550:35878) + (MasterF(19892:25220))*(MasterF(19892:25220))*Weight
        MasterF(35879) = MasterF(35879) + Weight
    end if
    ...
    if (UseMeasure(12)) then
        ...
        do SDCOr = 0, MasterI(187) - 1
            if (StepNum == MasterI(188) * SDCOr + MasterI(182)) then
                SDCPosOrigin(SDCOr, :, :) = Pos
            endif
            SDCInd = int((StepNum - MasterI(185) - SDCOr * MasterI(188)) / MasterI(182) - 1)
...
function modulehash()
    ...
    modulehash = '2f51e05eecfbbd0d62ab6b5f8edda840841a6a9b'
end function
